...
import { ForecastPoint } from '@shared/types/gpx-types';
  ...
  return (
    <div className="bg-card border border-border rounded-lg shadow-lg p-4 mb-4 animate-fade-in">
      ...
      <div className="mt-4 pt-3 border-t border-border">
        <div className="text-sm font-medium">{weather.conditionDescription}</div>
        <div className="text-xs text-muted-foreground mt-1">
          {weather.rain > 0 ? `Expect ${formatPrecipitation(weather.rain)} of rain. ` : ''}
          {weather.snow !== undefined && weather.snow > 0 ? `Expect ${formatPrecipitation(weather.snow)} of snow. ` : ''}
          {weather.windSpeed > 5 ? `Wind may be noticeable. ` : ''}
          {weather.uvIndex && weather.uvIndex > 5 ? `High UV index, sun protection recommended. ` : ''}
        </div>
      </div>
    </div>
  );
