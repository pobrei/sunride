...
import { RefreshCw, AlertTriangle, CloudRain, BarChart } from 'lucide-react';
...
const WeatherProviderComparison: React.FC = () => {
  ...
  // Function to fetch data from a specific provider
  const fetchProviderData = async (provider: string) => {
    ...
    try {
      ...
      const mockData = forecastPoints.map((point, index) => {
        const baseTemp = 15 + Math.sin(index * 0.5) * 10;
        const baseHumidity = 60 + Math.cos(index * 0.3) * 20;
        const baseWind = 10 + Math.sin(index * 0.7) * 8;
  ...
  const activeProviderData = providers.find(p => p.provider === activeProvider);

  // Check if we have data to display
  const hasData = providers.some(p => p.data.length > 0);

  // Calculate temperature differences if we have data from multiple providers
  const getTemperatureDifferences = () => {
    const providersWithData = providers.filter(p => p.data.length > 0);
    if (providersWithData.length <= 1) return null;

    // Use the first provider as the baseline
    const baseline = providersWithData[0];

    return providersWithData.slice(1).map(provider => {
      const avgDiff = provider.data.reduce((sum, data, index) => {
        ...
        {!hasData ? (
          <div className="space-y-4">
            ...
            <Button
              ...
              className="w-full bg-gradient-to-r from-primary to-secondary hover:opacity-90 transition-opacity shadow-sm h-11"
              size="lg"
            >
              {isComparing ? (
                <>
            ...
            {forecastPoints.length === 0 && (
              <div className="flex items-center p-4 bg-amber-500/10 border border-amber-200 dark:border-amber-800 rounded-xl shadow-sm">
