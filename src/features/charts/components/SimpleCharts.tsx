...
/**
 * A simplified version of the charts component that doesn't rely on Chart.js
 * @param props - Component props
 * @returns React component
 */
export default function SimpleCharts(props: SimpleChartsProps): React.ReactElement {
  ...
  const temperatures: number[] = weatherData
    .filter(w => w && w.temperature !== undefined)
    .map(w => w!.temperature!);

  const precipitations: number[] = weatherData
  ...
  const maxPrecip: number = Math.max(...precipitations);
