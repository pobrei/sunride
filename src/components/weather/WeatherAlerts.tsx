...
export function WeatherAlerts({
  ...
  useEffect(() => {
    if (!forecastPoints || !weatherData || forecastPoints.length === 0 || weatherData.length === 0) {
      setAlerts([]);
      return;
    }
  ...
  const displayedAlerts = showAll 
    ? visibleAlerts 
    : visibleAlerts.slice(0, maxInitialAlerts);
    ...
    <div className={cn('space-y-3', className)}>
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-medium flex items-center gap-2">
          ...
          Weather Alerts
          <span className="text-sm text-muted-foreground">({visibleAlerts.length})</span>
        </h3>
