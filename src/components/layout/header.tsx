  ...
  return (
    <motion.header
      className={cn(
        ...
        scrolled
          ? 'border-[#E5E7EB]/50 dark:border-[#2D3748]/50 shadow-[0_2px_5px_rgba(0,0,0,0.08)]'
    ...
    >
      <div className={cn(layout.flexRow, 'gap-2')}>
        ...
        <Link
          href="/"
          className={cn(layout.flexRow, 'gap-2 hover:opacity-90 transition-all duration-200')}
        >
          <motion.div
            className={cn(
              layout.flexCenter,
          ...
          <motion.span
            className={cn(
              'text-lg font-medium tracking-tight hidden sm:inline-block text-[#1A1A1A] dark:text-white'
            )}
      ...
      <div className={cn(layout.flexRow, 'gap-3')}>{/* Placeholder for optional elements */}</div>
    </motion.header>
  );
}
