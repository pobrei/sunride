      ...
      <div className="h-full w-full relative">
        ...
        <div className="absolute inset-0 grid grid-cols-4 grid-rows-4 pointer-events-none">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={`v-${i}`} className="border-r border-slate-300 dark:border-slate-700 h-full" style={{ left: `${(i + 1) * 25}%` }} />
          ))}
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={`h-${i}`} className="border-b border-slate-300 dark:border-slate-700 w-full" style={{ top: `${(i + 1) * 25}%` }} />
          ))}
        </div>
