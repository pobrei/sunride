...
const SafeChartsWrapper: React.FC<SafeChartsWrapperProps> = ({
  ...
  if (!hasValidData) {
    return (
      <div className={cn("relative rounded-xl overflow-hidden border border-border bg-transparent p-6", animation.fadeIn)}>
        <div className={cn("flex flex-col items-center justify-center text-center", animation.fadeInSlideUp)}>
          <AlertTriangle className="h-12 w-12 text-destructive mx-auto mb-4" />
          <h3 className={cn(typography.h4, "mb-2")}>Chart Data Error</h3>
          <p className={cn(typography.body, typography.muted, "mb-4")}>
            We couldn't display the charts because the data is invalid or incomplete.
          </p>
          <div className={cn(typography.bodySm, typography.muted, "mb-4")}>
