...
export function TripSummary({
  ...
  const tripDuration = forecastPoints[forecastPoints.length - 1].timestamp - forecastPoints[0].timestamp;
  ...
  const precipitationPoints = weatherData
    .filter((w): w is WeatherData => w !== null && w.precipitation > 0.2)
    .length;
  ...
  // Count alerts
  const alerts = {
    extremeHeat: weatherData.filter((w): w is WeatherData => w !== null && w.temperature > 35).length,
    ...
    heavyRain: weatherData.filter((w): w is WeatherData => w !== null && w.precipitation > 0.5).length
  };
        ...
        <div className="mt-4 pt-4 border-t border-border">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm">
              <span className="text-blue-500">{formatTemperature(minTemperature)}</span>
              {' — '}
              <span className="text-red-500">{formatTemperature(maxTemperature)}</span>
            </div>
          </div>
          ...
          <div className="mt-2 h-2 bg-muted rounded-full overflow-hidden relative">
            <div 
              className="absolute inset-y-0 left-0 bg-gradient-to-r from-blue-500 to-red-500 rounded-full"
              style={{ 
                width: '100%',
              }}
            />
            <div 
              ...
              style={{ 
                left: `${((avgTemperature - minTemperature) / (maxTemperature - minTemperature)) * 100}%`,
