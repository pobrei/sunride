import { WeatherData as BackendWeatherData } from './weatherAPI';
import { WeatherData as FullWeatherData } from './weatherService';

/**
 * Convert provider weather data to backend weather data format
 * @param data - Provider weather data
 * @returns Backend weather data
 */
function convertToBackendWeatherData(data: any): BackendWeatherData {
  return {
    ...
    weatherIcon: data.weatherIcon || '01d',
    weatherDescription: data.weatherDescription || 'Clear',
  };
}
 ...
 * @returns Backend weather data if valid, throws error if invalid
 */
function validateWeatherData(data: any): BackendWeatherData {
  if (!data) {
  ...
  return convertToBackendWeatherData(data);
}
...
interface WeatherProvider {
  name: string;
  getWeather(lat: number, lon: number, time: Date): Promise<BackendWeatherData>;
...
class OpenWeatherProvider implements WeatherProvider {
  ...
  async getWeather(lat: number, lon: number, time: Date): Promise<BackendWeatherData> {
    try {
...
export class WeatherService {
  ...
  private activeCalls = new Map<string, Promise<BackendWeatherData>>();
