...
import { ForecastPoint as SharedForecastPoint } from '@shared/types';
...
// Backend-specific forecast point interface
...
// Convert shared forecast point to backend forecast point
export function convertToBackendForecastPoint(point: SharedForecastPoint): ForecastPoint {
  return {
    lat: point.lat,
    lon: point.lon,
...
// Backend-specific weather data interface
...
export async function getMultipleForecastPoints(
  points: ForecastPoint[] | SharedForecastPoint[]
): Promise<(WeatherData | null)[]> {
  if (!points || !Array.isArray(points) || points.length === 0) {
    throw new Error('Invalid points array');
  }

  // Convert shared forecast points to backend forecast points if needed
  const backendPoints: ForecastPoint[] = points.map(point => {
    if ('timestamp' in point && typeof point.timestamp === 'number') {
      // Already a backend forecast point
      return point as ForecastPoint;
    } else {
      // Convert from shared forecast point
      return convertToBackendForecastPoint(point as SharedForecastPoint);
    }
  });

  // Process points in batches to avoid rate limiting
  const batchSize = 5;
  const results: (WeatherData | null)[] = new Array(backendPoints.length).fill(null);
  const MAX_RETRIES = 3;

  for (let i = 0; i < backendPoints.length; i += batchSize) {
    const batch = backendPoints.slice(i, i + batchSize);
...
// These are already exported as interfaces above
