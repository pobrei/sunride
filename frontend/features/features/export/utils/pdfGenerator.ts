...
import { WeatherData } from '@frontend/features/weather/types';
...
export async function generateWeatherReportPDF(
  ...
  if (mapImageBase64 && mapImageBase64.length < 5e6) {
    // Limit to 5MB
    try {
      const mapImageBytes = Buffer.from(
  ...
  for (let i = 0; i < weatherData.length; i += ITEMS_PER_PAGE) {
    const pageItems = weatherData.slice(i, i + ITEMS_PER_PAGE);
    ...
    // Draw table headers
    const headers = ['Time', 'Temperature', 'Wind', 'Precipitation', 'Humidity'];
