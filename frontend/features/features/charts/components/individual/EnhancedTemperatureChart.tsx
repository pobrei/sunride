...
import { formatDistance, formatTemperature, formatDateTime } from '@shared/utils/formatUtils';
...
const EnhancedTemperatureChart: React.FC<EnhancedTemperatureChartProps> = ({
  forecastPoints,
  weatherData,
  selectedMarker,
  onChartClick,
          ...
          <div className="h-[280px] w-full bg-muted/20 rounded-md flex items-center justify-center">
            <div className="text-center p-4">
              ...
              <div className="text-muted-foreground text-sm">
                {hasError ? 'Error loading temperature chart' : 'No temperature data available'}
              </div>
              {hasError && (
                <Button
                  variant="outline"
  ...
  const chartData: ChartDataPoint[] = forecastPoints.map((point, index) => {
    ...
    return {
      ...
      feelsLike: weather ? weather.feelsLike : null,
      index,
    };
  }).filter(point => point.temperature !== null);

  // Calculate min and max values for better chart display
  const temperatures = chartData.map(d => d.temperature).filter(Boolean) as number[];
  const feelsLike = chartData.map(d => d.feelsLike).filter(Boolean) as number[];
  const allTemps = [...temperatures, ...feelsLike];
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-background border border-border p-2 rounded-md shadow-md text-xs">
          ...
          {data.feelsLike !== null && data.feelsLike !== undefined && (
      ...
      <Card className={cn("overflow-hidden", className)}>
        ...
        <CardContent>
          <div className="h-[280px] w-full bg-muted/20 rounded-md flex items-center justify-center">
            <div className="text-center p-4">
              <RefreshCw className="h-8 w-8 mx-auto mb-2 text-muted-foreground animate-spin" />
              <div className="text-muted-foreground text-sm">Error rendering temperature chart</div>
              <Button
                variant="outline"
                size="sm"
                className="mt-4"
...
export default EnhancedTemperatureChart;
