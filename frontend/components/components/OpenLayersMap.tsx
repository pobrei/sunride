...
import { GPXData } from '@shared/utils/gpxParser';
...
import WeatherInfoPanel from '@frontend/components/WeatherInfoPanel';
...
const OpenLayersMap: React.FC<OpenLayersMapProps> = ({
    ...
    try {
      ...
      mapInstanceRef.current.on('click', event => {
        ...
        if (feature && feature.get('id') !== undefined) {
          const pointIndex = feature.get('id');
          onMarkerClick(pointIndex);

          // Set the selected point data for the info panel
          const point = feature.get('point');
          const weather = feature.get('weather');

          if (point && weather) {
            setSelectedPointData({ point, weather });
          }
        } else {
          // Clicked outside of a marker, close the info panel
  ...
  return (
    <div className="relative rounded-xl overflow-hidden border border-border bg-card card-shadow animate-fade-in transition-smooth">
      {/* Weather Info Panel */}
      {selectedPointData && (
        <WeatherInfoPanel
          point={selectedPointData.point}
          weather={selectedPointData.weather}
