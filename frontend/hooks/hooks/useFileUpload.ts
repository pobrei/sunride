...
import { useSimpleNotifications } from '@frontend/features/notifications/context';
import { ValidationError, tryCatch } from '@shared/utils';
import { captureException } from '@frontend/features/monitoring';
...
export function useFileUpload(options: FileUploadOptions = {}): {
      ...
      if (mergedOptions.allowedTypes && mergedOptions.allowedTypes.length > 0) {
        ...
        if (!isValidType) {
          throw new ValidationError(
            `Invalid file type. Allowed types: ${mergedOptions.allowedTypes.join(', ')}`
          );
        }
      }
  ...
  const uploadFile = useCallback(
    async (file: File, url: string): Promise<string> => {
      setState({
        isUploading: true,
        progress: 0,
        error: null,
        ...
        const uploadPromise: Promise<string> = new Promise<string>((resolve, reject) => {
          ...
          xhr.addEventListener('load', () => {
            if (xhr.status >= 200 && xhr.status < 300) {
              try {
                const response: { url?: string; fileUrl?: string; filePath?: string } = JSON.parse(
                  xhr.responseText
                );
   ...
   */
  const parseGPXFile = useCallback(
        ...
        (await tryCatch(
          ...
          error => {
            ...
            const errorObj =
              error instanceof Error
                ? error
                : new Error('An unknown error occurred while parsing the file');
            setState(prev => ({
              ...prev,
              error: errorObj,
            }));
