import { mergeWeatherWithRoute } from '@frontend/features/weather/utils/weatherDataMerging';
import { mockForecastPoints, mockWeatherData } from '../mocks/mockData';

// Mock the mergeWeatherWithRoute function if it doesn't exist
jest.mock('@frontend/features/weather/utils/weatherDataMerging', () => ({
  mergeWeatherWithRoute: jest.fn((forecastPoints, weatherData) => {
    ...
    return forecastPoints.map((point, index) => ({
      ...point,
      weather:
        weatherData && weatherData.length > 0 ? weatherData[index % weatherData.length] : null,
    }));
  }),
}));
