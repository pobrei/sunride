import { ForecastPoint } from '@shared/types';
import {
  ValidationError,
  GPXData,
  GPXPoint,
  RoutePoint,
  normalizeGPXData
} from '@shared/types/gpx-types';

// Export ValidationError for use in other files
...
export function parseGPX(gpxString: string): GPXData {
  ...
  // Convert RoutePoints to GPXPoints
  const gpxPoints: GPXPoint[] = routePoints.map(point => ({
    lat: point.lat,
    lon: point.lon,
    ele: point.elevation,
    time: point.time
  ...
  // Normalize the data to ensure backward compatibility
  return normalizeGPXData(gpxData);
}
...
export function generateForecastPoints(
  ...
  const totalDistance = gpxData.totalDistance || 0;
  ...
  for (let distance = interval; distance < totalDistance; distance += interval) {
    ...
    for (let i = 0; i < points.length - 1; i++) {
      // Get distance from the points array or calculate it
      const pointDistance = (points[i] as any).distance || 0;
    ...
    // Get distances for interpolation
  ...
  if (!lastAddedPoint || lastAddedPoint.distance !== totalDistance) {
    forecastPoints.push({
      lat: lastPoint.lat,
      lon: lastPoint.lon,
